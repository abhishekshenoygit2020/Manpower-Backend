...
module.exports = {
        ...
        pool.query(
            ...
            (err, results) => {
                ...
                if (err) {
                    ...
                    var password = "user" + Math.floor(Math.random() * 90000 + 10000);
                    pool.query(
                        `INSERT INTO login_master(firstname,lastname,user_email,user_type,user_password,status,date,dealership,ovmic_no) VALUES (?,?,?,?,?,?,?,?,?)`,
                        [
                            data.firstname,
                            data.lastname,
                            data.user_email,
    ...
    updateOVMICNO: (data, callBack) => {
        ...
        var block = 0;
        var message = "OMVIC No is Updated";

        if (action == 1) {
            block = 1;
            message = "OMVIC No is Updated";
        }
        pool.query(
            `UPDATE login_master SET ovmic_no=? WHERE  id = ?`,
            [
                ovmic_no,
                id
            ],
